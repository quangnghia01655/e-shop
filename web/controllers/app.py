...
def index():
    try:
        ...
        best_sellers = []
        try:
            sql = db.text('''SELECT p.id, p.name, p.price, p.image_url, SUM(oi.quantity) as total_sold
                     FROM products p
...
def login():
    ...
    if request.method == 'POST':
        ...
        return jsonify({'status': 'error', 'message': 'Thông tin đăng nhập không đúng'})
